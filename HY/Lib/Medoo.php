...
class Medoo
{
    ...
    public function exec($query, $map = [])
    {
        ...
        if (!$statement)
        {
            ...
            if (C('DEBUG_PAGE')) {
                $GLOBALS['SQL_LOG'][] = '异常：'.$this->generate($query, $map) . ' [耗时] ' . round(microtime(TRUE) - $a, 4) . 's';
        ...
        if (C('DEBUG_PAGE')) {
            $GLOBALS['SQL_LOG'][] = $this->generate($query, $map) . ' [耗时] ' . round(microtime(TRUE) - $a, 4) . 's';
        }
    ...
    protected function whereClause($where, &$map)
    {
        ...
        if (is_array($where))
        {
            if(isset($where['AND']) || isset($where['and'])){
                if(empty($where['AND'])) unset($where['AND']);
                if(empty($where['and'])) unset($where['and']);
            }
