<div class="item">
    ...
    <div class="middle text">
        <h4 class="title">
            ...
            {/if}
			<a style="{if $v['top']==2}font-weight: bold;color: #09C;{elseif $v['top']==1}font-weight: bold;color: #CE792D;{/if}" class="thread-title" href="{php HYBBS_URL('thread',$v['tid']);}">{if $v['top']==2}<span class="qzd">{$_LANG['全站置顶']}</span>{elseif $v['top']==1}<span class="fzd">{$_LANG['本版置顶']}</span>{/if}{$v.title}{if $v['state']}<span title="{$_LANG['禁止回复']}" style="color: brown;"> - {$_LANG['已锁定']}</span>{/if}</a>
